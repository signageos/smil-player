...
import { deterministicLastModifiedMs, fillWallclock } from './localServerTools';
...
export function createTestServer(serverPort: number = TestServer.port) {
	...
	const statusConfig: Record<string, number | 'timeout'> = {};
	...
	const recoverConfig: Record<string, boolean> = {};
	...
	app.get('/assets-b/:fileName', async (req, res) => {
		const filePath = path.join(process.env.PWD!, TestServer.assetsPath, req.params.fileName);
		try {
		...
		res.set(
			'Last-Modified',
			new Date(deterministicLastModifiedMs(req.params.fileName) + 3_600_000).toUTCString(),
		);
	...
	app.get('/dedup/content/shared.jpg', async (req, res) => {
		const buf = await fs.readFile(`./${TestServer.assetsPath}/landscape1.jpg`);
	...
	async function getCbpVideoBuffer(): Promise<Buffer> {
		if (!cbpVideoBuffer) {
			cbpVideoBuffer = await fs.readFile(`./${TestServer.assetsPath}/loader.mp4`);
		}
	...
	const gateSkipMode = new Set<string>();
	// Forced status per name (e.g. 500 for the fail-open flow). 0/absent = no override.
	const gateStatusOverride: Record<string, number> = {};
	...
	app.post('/gate/status-override/:name', (req, res) => {
		...
		res.set(CBP_CORS_HEADERS);
		res.json({ name, statusOverride: gateStatusOverride[name] ?? null });
	});
	...
	app.post('/gate/delay/:name', (req, res) => {
		...
		res.set(CBP_CORS_HEADERS);
		res.json({ name, delayMs: gateDelayMs[name] ?? null });
	});
	...
	app.head('/cbp-loc/image.png', (_req, res) => {
		...
		res.set({
			...
			'Content-Type': 'image/png',
			'Content-Length': String(png.length),
			'Location': `http://localhost:${port}/cbp-loc/content/image_v${cbpLocVersion}.png`,
		});
		res.status(200).end();
	});
	...
	app.head('/cbp-loc/:name', (req, res) => {
		...
		res.set({
			...
			'Content-Type': 'image/png',
			'Content-Length': String(png.length),
			'Location': `http://localhost:${port}/cbp-loc/content/${baseName}_v${img.version}.png`,
		});
		res.status(200).end();
	});
	...
	app.head('/cbp-loc-noext/content', (req, res) => {
		...
		res.set({
			...CBP_CORS_HEADERS,
			'Location': `http://localhost:${port}/cbp-loc-noext/file/video_v${id}.mp4`,
		});
	...
	const isRecoverBroken = (fileName: string): boolean => recoverConfig[fileName] !== false;
	...
	app.use(
		express.static(path.join(process.env.PWD!, TestServer.testFilesPath), {
			lastModified: false,
			setHeaders: (res: any, filePath: string) => {
				res.set('Last-Modified', new Date(deterministicLastModifiedMs(path.basename(filePath))).toUTCString());
			},
		}),
	);
