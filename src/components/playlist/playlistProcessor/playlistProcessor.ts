...
import {
	...
	SMILMediaNoVideo,
	SMILVideo,
	SosHtmlElement, VideoParams,
} from "../../../models/mediaModels";
import {
	debug,
	generateParentId, getDefaultVideoParams,
...
export class PlaylistProcessor extends PlaylistCommon implements IPlaylistProcessor {
	...
	public playIntro = async (
		...
		downloadPromises = downloadPromises.concat(
			await this.files.parallelDownloadAllFiles(this.internalStorageUnit, [<SMILVideo | SMILImage> smilObject.intro[0][media]], fileStructure),
		);
			...
			if (!isNil(this.sos.config.syncServerUrl) && isUrl(this.sos.config.syncServerUrl)) {
				...
				for (let [key, value] of Object.entries(smilObject.region)) {
					if (value.sync) {
						...
						await this.sos.sync.init(`${this.synchronization.syncGroupName}-${key}`, this.synchronization.syncDeviceId);
						initCalled = true;
					}
				}
				if (!initCalled) {
					await this.sos.sync.init(`${this.synchronization.syncGroupName}`, this.synchronization.syncDeviceId);
				}
	...
	public processPlaylist = async (
		...
		for (let [key, loopValue] of Object.entries(playlist)) {
			...
			if (XmlTags.extractedElements.includes(removeDigits(key))) {
				...
				} = await this.priority.priorityBehaviour(<SMILMedia> value, version, parent, endTime, priorityObject);
				await this.playElement(
					<SMILMedia> value, version, key, parent, currentIndex, previousPlayingIndex, endTime, isLast);
				continue;
			}
			...
			if (key === 'par') {
				...
				if (value.hasOwnProperty('begin') && value.begin!.indexOf('wallclock') > -1) {
					...
					if (value.repeatCount === 'indefinite') {
						promises.push(
							this.createRepeatCountIndefinitePromise(value, priorityObject, version, parent, timeToEnd, key, conditionalExpr, timeToStart),
						);
						await Promise.all(promises);
						continue;
					}
						...
						if (valueElement.repeatCount === 'indefinite') {
							promises.push(
								this.createRepeatCountIndefinitePromise(
									valueElement, priorityObject, version, parent, timeToEnd, key, conditionalExpr, timeToStart),
							);
	...
	private createRepeatCountIndefinitePromise = (
		...
		conditionalExpr: string = '', timeToStart: number = -1,
	): Promise<void> => {
		return ((async () => {
	...
	private checkRegionsForCancellation = async (
		...
		if (parentRegion.regionName !== regionInfo.regionName && this.currentlyPlaying[parentRegion.regionName]?.playing) {
		...
		if (this.currentlyPlaying[SMILEnums.defaultRegion]?.src !== element.src && this.currentlyPlaying[SMILEnums.defaultRegion]?.playing) {
		...
		if (this.currentlyPlaying[regionInfo.regionName]?.src !== element.src && this.currentlyPlaying[regionInfo.regionName]?.playing) {
			...
			if (!isNil(this.currentlyPlaying[regionInfo.regionName][SMILTriggersEnum.triggerValue]) && !isNil(element.triggerValue)
				&& this.currentlyPlaying[regionInfo.regionName][SMILTriggersEnum.triggerValue] !== element.triggerValue) {
				debug('cancelling trigger: %s from element: %s', this.currentlyPlaying[regionInfo.regionName].src, element.src);
				let triggerValueToCancel = findTriggerToCancel(this.triggers.triggersEndless, regionInfo.regionName, element.triggerValue);
				// stop trigger
	...
	private playHtmlContent = async (
		...
		currentIndex: number, endTime: number, isLast: boolean, regionInfo: RegionAttributes,
		...
		try {
			...
			this.promiseAwaiting[regionInfo.regionName].promiseFunction! = [(async () => {
				...
				await this.waitMediaOnScreen(
					regionInfo, parentRegion, sosHtmlElement, arrayIndex, element, transitionDuration, taskStartDate, version,
				);
				debug('Finished iteration of playlist: %O', this.currentlyPlayingPriority[regionInfo.regionName][currentIndex]);
				this.priority.handlePriorityWhenDone(regionInfo.regionName, currentIndex, endTime, isLast, version, this.playlistVersion);
				debug('Finished checking iteration of playlist: %O', this.currentlyPlayingPriority[regionInfo.regionName][currentIndex]);
	...
	private waitMediaOnScreen = async (
		regionInfo: RegionAttributes, parentRegion: RegionAttributes, element: SosHtmlElement, arrayIndex: number,
	...
	private shouldWaitAndContinue = async (
		media: SMILMedia, regionInfo: RegionAttributes, parentRegionName: string,
		...
		if (this.currentlyPlayingPriority[parentRegionName][previousPlayingIndex].behaviour !== 'pause'
		...
		if (media.hasOwnProperty(SMILTriggersEnum.triggerValue) && !this.triggers.triggersEndless[media.triggerValue as string]?.play) {
		...
		if (currentIndexPriority?.player.stop || currentIndexPriority?.player.contentPause !== 0
			|| currentIndexPriority?.behaviour === 'pause') {
			debug(
				'Playlist was stopped/paused by higher priority during await: %O', currentIndexPriority,
			);
			return false;
		}
	...
	private playVideo = async (
		video: SMILVideo, version: number, currentIndex: number, endTime: number, isLast: boolean, regionInfo: RegionAttributes, index: number,
		parentRegion: RegionAttributes, sosVideoObject: Video | Stream, params: VideoParams,
	) => {
		const taskStartDate = moment().toDate();
	...
	private handleVideoPlay = async (
		video: SMILVideo, params: VideoParams, sosVideoObject: Video | Stream, regionInfo: RegionAttributes,
		parentRegion: RegionAttributes, version: number) => {
		let promiseRaceArray = [];
		params.pop();
		if (this.currentlyPlaying[regionInfo.regionName]?.src !== video.src && this.currentlyPlaying[regionInfo.regionName]?.playing
	...
	private handleStreamPlay = async (
		stream: SMILVideo, params: VideoParams, sosVideoObject: Video | Stream, regionInfo: RegionAttributes,
		parentRegion: RegionAttributes, version: number) => {
		let promiseRaceArray = [];
	...
	private setupIntroVideo = async (video: SMILVideo, region: RegionsObject) => {
		const currentVideoDetails = <IFile> await this.files.getFileDetails(video, this.internalStorageUnit, FileStructure.videos);
	...
	private setupIntroImage = async (
		image: SMILImage, region: RegionsObject, key: string,
	): Promise<HTMLElement> => {
		const currentImageDetails = <IFile> await this.files.getFileDetails(image, this.internalStorageUnit, FileStructure.images);
		...
		const element: HTMLElement = createHtmlElement(HtmlEnum.img, image.localFilePath, image.regionInfo, key, image.src);
		image.id = element.id;
		element.style.visibility = 'visible';
	...
	private playElement = async (
		value: SMILMedia, version: number, key: string, parent: string,
		...
		if (!(await this.shouldWaitAndContinue(
			value, regionInfo, parentRegion.regionName, currentIndex, previousPlayingIndex, endTime, isLast, version,
		))) {
			return;
		}
		...
		switch (removeDigits(key)) {
			case 'video':
				await this.playVideo(
					value, version, currentIndex, endTime, isLast, regionInfo, index,
					parentRegion, sosVideoObject, params,
				);
				break;
			case 'ref':
				await this.playHtmlContent(value as SMILMediaNoVideo, version, index, currentIndex, endTime, isLast, regionInfo, parentRegion);
				break;
			case SMILEnums.img:
				await this.playHtmlContent(value as SMILMediaNoVideo, version, index, currentIndex, endTime, isLast, regionInfo, parentRegion);
				break;
	...
	private handleVideoPrepare = async (value: SMILVideo, regionInfo: RegionAttributes): Promise<{
		sosVideoObject: Video | Stream, params: VideoParams,
	} | undefined> => {
		...
		const params: VideoParams =
			[videoPath, regionInfo.left, regionInfo.top, regionInfo.width, regionInfo.height, options as keyof typeof StreamProtocol];
		...
		if ((this.currentlyPlaying[regionInfo.regionName]?.src !== value.src
				&& this.videoPreparing[regionInfo.regionName]?.src !== value.src)
			|| (config.videoOptions.background && value.protocol !== StreamEnums.internal
				&& this.videoPreparing[regionInfo.regionName]?.src !== value.src)) {

			if (!await this.files.fileExists(this.internalStorageUnit, createLocalFilePath(FileStructure.videos, value.src))) {
	...
	private handleHtmlElementPrepare = (value: SMILMediaNoVideo, element: HTMLElement, version: number) => {
		...
		if ((element.getAttribute('src') === null
			|| element.getAttribute('src') !== src) && value.preload !== false) {
			element.setAttribute('src', src);
		}
	}
	...
	private handleElementSynchronization = async (value: SMILMedia): Promise<boolean> => {
		if (value.regionInfo.sync && this.synchronization.shouldSync) {
			...
			if (isNil(this.synchronization.syncValue) ||
				isEqual(value.syncIndex, this.synchronization.syncValue) || this.synchronization.movingForward) {
				if (!isNil(this.synchronization.syncValue) &&
					isEqual(value.syncIndex, this.synchronization.syncValue) && this.synchronization.syncingInAction) {
					//move one forward
				...
				currentSyncIndex = await this.sos.sync.wait(value.syncIndex, `${this.synchronization.syncGroupName}-${value.regionInfo.regionName}`);
