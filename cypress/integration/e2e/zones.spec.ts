...
describe("zonesCypress.smil test", () => {
	it("processes smil file correctly", () => {
		...
		cy.iframe().find('img[src*="images/widget_ima_011645d5.png"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('be.visible');
		cy.iframe().find('iframe[src*="widgets/extracted/bottomWidg_f23be3fb.wgt"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('be.visible');
		cy.iframe().find('img[src*="images/widget_ima_db3167f1.png"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('be.visible');
		...
		testCoordinates(cy.iframe().find('iframe[src*="widgets/extracted/bottomWidg_f23be3fb.wgt"]'), 1080 - 360, 0, 1280, 360);
