import { testCoordinates, testInvisible, testVisible, doesNotExist } from '../../tools/tools';
...
describe("zonesCypress.smil test", () => {
	it("processes smil file correctly", () => {
		...
		cy.iframe().find('img[src*="images/widget_image_1.png"]', { timeout: 10000 }).should('be.visible');
		...
		testVisible(cy.iframe().find('img[src*="images/img_1.jpg"]'));
		testCoordinates(cy.iframe().find('img[src*="images/img_1.jpg"]'), 0, 0, 1280, 720);
		cy.wait(CypressTimeouts.videoTransitionTimeout);

		testInvisible(cy.iframe().find('img[src*="images/img_1.jpg"]'));
		testVisible(cy.iframe().find('img[src*="images/img_2.jpg"]'));
		...
		testCoordinates(cy.iframe().find('iframe[src*="widgets/extracted/bottomWidget.wgt"]'), 1080 - 360, 0, 1280, 360);
		testCoordinates(cy.iframe().find('img[src*="images/widget_image_2.png"]'), 506, 1280, 640, 574);
		testVisible(cy.iframe().find('img[src*="images/widget_image_1.png"]'));
		testVisible(cy.iframe().find('iframe[src*="widgets/extracted/bottomWidget.wgt"]'));
		testVisible(cy.iframe().find('img[src*="images/widget_image_2.png"]'));
	});
});
