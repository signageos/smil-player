...
describe("videoStreams.smil test", () => {
	it("processes smil file correctly", () => {
		...
		cy.iframe().find('img[id*="landscape1_7a8cff48.jpg-top-right-img1"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('be.visible');
		testCoordinates(cy.iframe().find('img[id*="landscape1_7a8cff48.jpg-top-right-img1"]'), 0, 960, 960, 540);

		cy.iframe().find('img[id*="landscape1_7a8cff48.jpg-top-right-img1"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('not.be.visible');
		...
		cy.get('video[src*="https://www.rmp-streaming.com/media/bbb-360p.mp4"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('be.visible');
		testCoordinates(cy.get('video[src*="https://www.rmp-streaming.com/media/bbb-360p.mp4"]'), 0, 960, 960, 540);

		cy.get('video[src*="https://www.rmp-streaming.com/media/bbb-360p.mp4"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('not.be.visible');
		cy.iframe().find('img[id*="landscape1_7a8cff48.jpg-top-right-img1"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should('be.visible');
		testCoordinates(cy.iframe().find('img[id*="landscape1_7a8cff48.jpg-top-right-img1"]'), 0, 960, 960, 540);
	});
});
