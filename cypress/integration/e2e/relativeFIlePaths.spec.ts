...
describe('relativeFilePaths.smil test', () => {
	it('processes smil file correctly', () => {
		...
		cy.get('video[src*="videos/loader_a667ec98.mp4"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should(
			'be.visible',
		);
		testCoordinates(cy.get('video[src*="videos/loader_a667ec98.mp4"]'), 0, 0, 1920, 1080);
		cy.get('video[src*="videos/landscape1_86c12946.mp4"]', { timeout: CypressTimeouts.elementAwaitTimeout }).should(
			'be.visible',
		);
