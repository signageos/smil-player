...
Note: a syntactically valid SMIL file with an **empty playlist** is treated as an intentional state — the player shows
nothing and does **not** fall back to the backup image. The backup image appears only for download or parse failures.
See also the `fallbackToPreviousPlaylist` option in [Updating Content](updating-content.md), which keeps
the previous playlist running instead of showing the backup image.
...
![Set backup image in applet configuration](../../assets/backupImage-timings.png)
