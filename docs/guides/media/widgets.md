...
rendered as a [live HTML5 website](websites.md) instead. The `type` attribute is conventional and not used for
this decision.
...
See [`preload`](../../reference/element-attributes.md#preload).
...
By default, the `sos` object is undefined and the JS SDK cannot be used. In case you do not have the `sos` object
available, you need to manually add it to your project by one of the following options:
