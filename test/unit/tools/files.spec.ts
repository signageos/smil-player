...
describe('Files tools component', () => {
	...
	describe('Files tools component getFileName extension-borrow (fallbackUrlForExt)', () => {
		// pathname `/endpoint` has no extension — e.g. an API endpoint that serves
		// the real file via a Location header. This is the only shape that triggers
		// the borrow. The whole reason locationStrategyExtension.test.ts exists.
		const endpoint = 'https://api.example.com/endpoint';

		it('borrows the extension from a resolved Location fallback when the primary URL has none', () => {
			const borrowed = getFileName(endpoint, 'https://cdn.example.com/video.mp4');
			// stem + 8-char host checksum come from the PRIMARY url; the extension from the fallback
			expect(borrowed).to.match(/^endpoint_[0-9a-f]{8}\.mp4$/);
			expect(borrowed).to.be.equal(`${getFileName(endpoint)}.mp4`);
		});

		it('host-guard: a Last-Modified date string cannot supply a fake extension', () => {
			const lastModified = 'Wed, 21 Oct 2015 07:28:00 GMT';
			expect(getFileName(endpoint, lastModified)).to.be.equal(getFileName(endpoint));
		});

		it('host-guard: a hostless string with a real-looking extension is still rejected', () => {
			// "video.mp4" parses to host=null, so even a valid-looking extension must not be borrowed
			expect(getFileName(endpoint, 'video.mp4')).to.be.equal(getFileName(endpoint));
		});

		it('does not borrow when the primary URL already has an extension', () => {
			const withExt = 'https://cdn.example.com/video.mp4';
			const result = getFileName(withExt, 'https://other.com/poster.png');
			expect(result).to.match(/\.mp4$/);
			expect(result).to.be.equal(getFileName(withExt));
		});

		it('does not borrow when the fallback URL has a host but no extension', () => {
			expect(getFileName(endpoint, 'https://cdn.example.com/path')).to.be.equal(getFileName(endpoint));
		});
	});
	...
	describe('shouldGateSkipForStatus', () => {
		it('should skip when status is listed', () => {
			expect(shouldGateSkipForStatus(404, [403, 404, 410])).to.be.equal(true);
		});

		it('should play on 200 and 304 when not listed', () => {
			expect(shouldGateSkipForStatus(200, [403, 404, 410])).to.be.equal(false);
			expect(shouldGateSkipForStatus(304, [403, 404, 410])).to.be.equal(false);
		});

		it('should fail-open on unlisted 5xx', () => {
			expect(shouldGateSkipForStatus(500, [403, 404, 410])).to.be.equal(false);
			expect(shouldGateSkipForStatus(503, [403, 404, 410])).to.be.equal(false);
		});

		it('should skip on a listed 503 (explicit list wins over fail-open)', () => {
			expect(shouldGateSkipForStatus(503, [503])).to.be.equal(true);
		});

		it('should never skip with an empty list (gate inert)', () => {
